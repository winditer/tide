...
`trigger_type` 取值受限于：`cron` | `interval` | `date`（在 [`ScheduleCreate`](file:///Users/haifeng/Documents/tide/backend/models/schemas.py#L161-L168) 中校验）。
...
`task_type` 取值受限于：`agent` | `plan` | `workflow` | `status` | `custom`（同样在 [`ScheduleCreate`](file:///Users/haifeng/Documents/tide/backend/models/schemas.py#L161-L168) 中校验）。每种类型的 `task_config` 字段不同。
...
灵活的兜底类型，支持自定义脚本命令或 webhook 回调。当前后端实现也是复用 Agent 通道（把 `prompt` 派发给指定 Agent），并允许通过 `script` / `webhook_url` 等字段携带自定义参数，由 Agent 或下游适配器执行。`task_config` 字段示例：
...
所有接口前缀为 `/api/schedules`，定义见 [`backend/api/schedules.py`](file:///Users/haifeng/Documents/tide/backend/api/schedules.py)。
