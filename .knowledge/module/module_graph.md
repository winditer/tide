...
> 生成时间: 2026-06-26T02:33:51Z | 模块数: 96 | 依赖边数: 319
...
| `dashboard` | `backend/api/dashboard.py` | 1149 | 7 | 1 |  |
| `events` | `backend/api/events.py` | 69 | 2 | 1 | 事件补偿 API — 重连后拉取缺失事件 |
| `files` | `backend/api/files.py` | 328 | 3 | 1 | 文件读取 API 路由。 |
| `git_audit` | `backend/api/git_audit.py` | 957 | 4 | 1 | Git 审计 API 路由。 |
| `hooks` | `backend/api/hooks.py` | 140 | 2 | 1 | Hooks 事件驱动 API 路由。 |
| `kanban` | `backend/api/kanban.py` | 194 | 5 | 1 | Kanban 聚合 API 路由。 |
| `knowledge` | `backend/api/knowledge.py` | 254 | 2 | 1 | 知识图谱 API。 |
...
| `projects` | `backend/api/projects.py` | 891 | 10 | 1 | 项目 API 路由。 |
...
| `sessions` | `backend/api/sessions.py` | 836 | 8 | 1 | 会话 API：合并 SQLite tasks 表（按 session_id 聚合）+ 本地 Agent 会话文件扫描。 |
| `skills` | `backend/api/skills.py` | 147 | 2 | 1 | Skills 知识库 API 路由。 |
| `tasks` | `backend/api/tasks.py` | 450 | 6 | 1 | 任务 API 路由。 |
...
| 模块 | 文件 | 行数 | 内部依赖 | 被依赖 | 说明 |
|------|------|------|---------|--------|------|
| `dependencies` | `backend/core/dependencies.py` | 293 | 4 | 28 | FastAPI 认证依赖注入。 |
...
| 模块 | 文件 | 行数 | 内部依赖 | 被依赖 | 说明 |
|------|------|------|---------|--------|------|
| `engine` | `backend/db/engine.py` | 171 | 0 | 60 |  |
...
| `adapters` | `backend/runtime/adapters.py` | 1293 | 6 | 7 | Agent 适配器：AgentAdapter 基类及 Codex / Claude / Qoder 三个子类。 |
| `config` | `backend/runtime/config.py` | 175 | 0 | 20 | 环境变量配置与常量。 |
| `executor` | `backend/runtime/executor.py` | 703 | 6 | 6 | AgentExecutor: 异步 Agent CLI 执行器 |
| `git_utils` | `backend/runtime/git_utils.py` | 1019 | 0 | 7 | Git 异步操作工具模块。 |
...
| 模块 | 文件 | 行数 | 内部依赖 | 被依赖 | 说明 |
|------|------|------|---------|--------|------|
| `backfill_tokens` | `backend/scripts/backfill_tokens.py` | 116 | 2 | 0 | 回填历史任务的 token 估算数据。 |
...
| `ai_decompose_service` | `backend/services/ai_decompose_service.py` | 340 | 2 | 1 | AI 需求分解服务。 |
...
| `card_action_handler` | `backend/services/card_action_handler.py` | 647 | 7 | 1 | CardActionHandler — Lark 卡片按钮回调分发。 |
...
| `cost_service` | `backend/services/cost_service.py` | 301 | 1 | 5 | Token 成本计算与统计服务。 |
...
| `kanban_service` | `backend/services/kanban_service.py` | 948 | 5 | 1 | KanbanService — 四维看板数据聚合。 |
| `knowledge_service` | `backend/services/knowledge_service.py` | 631 | 2 | 1 | 知识图谱服务。 |
...
| `message_handler` | `backend/services/message_handler.py` | 1896 | 18 | 1 | MessageHandler: Lark 消息指令处理器 |
...
| `project_discovery` | `backend/services/project_discovery.py` | 339 | 0 | 4 | 项目发现服务 - 通过扫描本地 Agent 会话文件发现项目 |
...
| `session_discovery` | `backend/services/session_discovery.py` | 793 | 4 | 8 | 会话发现服务 - 从本地 Agent 会话文件提供历史会话/任务数据。 |
| `skill_service` | `backend/services/skill_service.py` | 353 | 1 | 3 | SkillService — Skills 知识库 CRUD + 批量导入。 |
| `task_service` | `backend/services/task_service.py` | 926 | 10 | 12 | TaskService — 任务 CRUD + Agent 执行桥接层。 |
| `work_item_service` | `backend/services/work_item_service.py` | 2635 | 7 | 10 | WorkItemService — 工作项 CRUD + 流转引擎。 |
...
| `test_lark_permission` | `backend/tests/test_lark_permission.py` | 237 | 2 | 0 | Lark 权限过滤单元测试 |
