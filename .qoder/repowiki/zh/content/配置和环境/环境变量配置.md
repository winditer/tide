...
## 简介
本文件系统性梳理 Tide 的环境变量配置体系，覆盖 Lark 应用、Agent CLI、访问控制、卡片消息、审批、状态持久化与 macOS 保活等全部配置项。文档提供默认值、用途说明、使用示例与最佳实践，并解释配置加载顺序、优先级与覆盖规则，附带验证方法、常见错误与解决方案，以及不同使用场景的推荐配置模板。
...
```mermaid
graph TB
A["tide_ws.py<br/>主程序入口"] --> B[".env<br/>环境变量文件"]
...
图表来源
- [tide_ws.py:33-47](file://tide_ws.py#L33-L47)
- [tide_ws.py:8531-8557](file://tide_ws.py#L8531-L8557)
- [tide_ws.py:8559-8584](file://tide_ws.py#L8559-L8584)
...
- [tide_ws.py:56-59](file://tide_ws.py#L56-L59)
...
- [tide_ws.py:62-98](file://tide_ws.py#L62-L98)
- [tide_ws.py:557-613](file://tide_ws.py#L557-L613)
...
- [tide_ws.py:1229-1254](file://tide_ws.py#L1229-L1254)
...
- PLAN_WORKTREE_ROOT
  - 默认值：空
  - 说明：自定义 worktree 根目录；为空时使用当前仓库的 .tide/worktrees
- PLAN_TEST_COMMAND
  - 默认值：git diff --check
...
- [tide_ws.py:125-149](file://tide_ws.py#L125-L149)
- [tide_ws.py:8548-8552](file://tide_ws.py#L8548-L8552)
...
## 结论
Tide 的环境变量配置体系覆盖了从 Lark 应用接入、Agent CLI 执行、访问控制、消息与卡片处理、审批流程、状态持久化到 macOS 保活的全链路需求。遵循 .env 与系统环境变量的加载顺序与优先级，配合 validate_config() 的校验与 README 的推荐模板，可在不同场景下实现稳定高效的运行。