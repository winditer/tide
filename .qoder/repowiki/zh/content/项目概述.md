...
## 简介
Lark2Agent 是一个将飞书/Lark 群聊消息转化为 Codex、Claude Code、Qoder CLI 等 Agent CLI 任务的轻量桥接工具。其核心价值在于：
...
```mermaid
graph TB
A["仓库根目录"] --> B["lark2agent_ws.py<br/>主程序"]
...
## 架构总览
Lark2Agent 采用“Lark WebSocket + Python Bridge + Agent CLI”的轻量架构：
...
AD --> CLI["Agent CLI"]
CARD --> WS
RT --> STATE["状态文件(.lark2agent_state.json)"]
STATE --> RT
```
...
## 结论
Lark2Agent 通过轻量桥接，将本地 Agent CLI 的工程能力与 Lark 的协作入口深度融合，解决了“本地执行与协作入口割裂”的痛点。其核心优势在于：
- 低侵入：不改变现有 Agent CLI 使用方式，不迁移开发环境。
- 协作可见：长任务进度与结果在 Lark 可见，审批动作闭环。
- 可并行：Plan 模式支持多任务并行与 Git 隔离。
- 可追踪：任务卡片与状态持久化，便于回溯与恢复。

在现代软件开发工作流中，Lark2Agent 既适合工程团队的远程协作，也适合个人开发者在移动端触发与跟踪任务。未来可围绕状态持久化、Plan 收尾与多用户部署持续增强。
...
- 与其他方案对比
  - 相比 OpenClaw（多渠道网关）与 Hermes（自改进 agent runtime），Lark2Agent 更聚焦“Lark + Agent CLI 的工程执行闭环”，不做泛化平台改造，保持简单与高效。