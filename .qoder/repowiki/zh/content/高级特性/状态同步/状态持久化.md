...
## 简介
本文件围绕 .tide_state.json 状态持久化机制进行系统化说明，涵盖状态文件的格式与结构、数据序列化方案、版本兼容性处理、文件读写策略，以及状态数据的组织方式（RUNTIMES、TASKS、DESKTOP_SYNCS、PENDING_APPROVALS、PLANS 等）。文档还解释了状态文件的加载与保存机制（启动时恢复、运行时更新、异常保护）、备份与迁移方法、损坏时的故障恢复策略与调试技巧。

## 项目结构
- 状态文件位置与命名
  - 状态文件默认名为 .tide_state.json，可通过环境变量 TIDE_STATE_FILE 指定。
  - 文件位于当前工作目录，用于保存已知聊天、运行面板状态、审批记录与 Plan/子任务状态等。
- 关键模块与职责
  - 主程序入口负责初始化配置、建立 Lark 连接、维护全局运行时对象与事件循环。
  - 状态持久化模块负责读取/写入 .tide_state.json，保证跨进程重启后状态可恢复。
...
**图表来源**
- [tide_ws.py:126](file://tide_ws.py#L126)
- [tide_ws.py:387-396](file://tide_ws.py#L387-L396)
...
- 状态文件读写接口
  - read_state：读取 .tide_state.json，返回字典结构；若文件不存在或损坏，返回空字典以便后续初始化。
  - write_state：将内存中的状态整体写回 .tide_state.json，采用原子写策略（先写临时文件，再重命名为正式文件），降低损坏风险。
...
**章节来源**
- [tide_ws.py:387-396](file://tide_ws.py#L387-L396)
- [tide_ws.py:988-1014](file://tide_ws.py#L988-L1014)
- [tide_ws.py:1017-1041](file://tide_ws.py#L1017-L1041)
...
participant State as "状态文件(.tide_state.json)"
...
- 数据结构概览
  ...
  - 其他全局状态（如 PENDING_APPROVALS、DESKTOP_SYNCS、TASKS）在当前代码中未直接持久化到 .tide_state.json，但会在运行时维护，重启后可通过 chats/plans 等恢复关键上下文。
- 版本兼容性
...
- TASKS
  ...
  - 写入时机：当前代码未直接持久化 TASKS 到 .tide_state.json，但可通过其他机制（如任务卡片状态）间接影响持久化。
- DESKTOP_SYNCS
  ...
  - 写入时机：当前代码未直接持久化 DESKTOP_SYNCS 到 .tide_state.json。
- PENDING_APPROVALS
  ...
  - 写入时机：当前代码未直接持久化 PENDING_APPROVALS 到 .tide_state.json。
- PLANS
...
- [tide_ws.py:298-309](file://tide_ws.py#L298-L309)
...
### 备份与迁移方法
- 手动备份步骤
  - 在停止服务或确认无写入时，复制 .tide_state.json 到安全位置。
  - 建议同时备份当前工作目录下的 .tide/attachments 等相关数据，便于完整恢复。
- 版本升级注意事项
  - 若未来引入新字段，建议在 read_state 时提供默认值，避免因字段缺失导致恢复失败。
  - 若字段结构发生重大变化，应在升级脚本中提供迁移逻辑，将旧格式转换为新格式后再写入。
- 数据恢复流程
  - 停止服务，将备份文件替换为 .tide_state.json，启动服务后 load_known_chats 会自动恢复 RUNTIMES/PLANS 等状态。
...
- 恢复策略
  - 备份优先：若存在 .tide_state.json.bak 或其他备份，优先使用备份恢复。
...
- 调试技巧
  ...
  - 使用最小化配置启动，排除第三方因素干扰。
  - 检查文件权限与磁盘空间，确保 .tide_state.json 可正常读写。

**章节来源**
- [tide_ws.py:868-878](file://tide_ws.py#L868-L878)

## 依赖关系分析
- 组件耦合
  - RUNTIMES/TASKS/PLANS 等内存状态与 .tide_state.json 之间通过 save_runtime/save_plans_state/read_state/write_state 解耦。
  - DESKTOP_SYNCS/PENDING_APPROVALS 当前未直接持久化，但可通过面板与命令间接影响持久化分区。
- 外部依赖
  - 环境变量 TIDE_STATE_FILE 控制状态文件路径。
  - 日志系统用于记录状态读写与异常信息。

```mermaid
graph LR
Env["环境变量 TIDE_STATE_FILE"] --> StateFile[".tide_state.json"]
...
## 结论
.tide_state.json 是 Tide 的核心状态载体，通过 chats/plans 分区保存关键运行上下文，配合原子写策略与降级回退机制，实现了可靠的跨进程重启恢复能力。尽管部分运行时状态（如 TASKS、DESKTOP_SYNCS、PENDING_APPROVALS）未直接持久化，但通过面板与命令仍可重建关键业务状态。建议在升级与运维中重视备份与迁移策略，确保生产环境的稳定性与可恢复性。

## 附录
- 环境变量参考
  - TIDE_STATE_FILE：状态文件路径
  - TIDE_SHOW_ARCHIVED：是否显示归档内容
  - TIDE_INCLUDE_PLAN_WORKTREES：是否将 Plan worktree 作为项目统计
  - TIDE_WELCOME_MESSAGE：启动欢迎语
  - SYNC_DESKTOP_SESSIONS：是否监听 Codex Desktop 会话
  - SESSION_WATCH_INTERVAL_SECONDS：会话监听间隔
- 文件说明
  - .tide_state.json：运行时状态文件，本地生成，不提交。
