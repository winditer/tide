...
## 简介
本指南面向 Lark2Agent 项目的开发者，提供从环境准备、依赖安装、IDE 配置、开发工具链到版本控制与工作流的最佳实践。内容基于仓库中的 README、DEVELOPMENT 与 ROADMAP 文档，以及主程序 lark2agent_ws.py 的实现细节，帮助你快速搭建可运行的开发环境并高效开展迭代。

## 项目结构
仓库采用“单脚本 + 环境变量 + 状态文件”的轻量结构，核心文件与职责如下：
- lark2agent_ws.py：主程序，负责 Lark WebSocket 事件、消息卡片、Agent CLI 执行、审批、会话同步与 macOS 保活。
...
```mermaid
graph TB
A["lark2agent_ws.py<br/>主程序"] --> B["Lark SDK<br/>事件与卡片"]
A --> C["Agent CLI<br/>Codex/Claude/Qoder"]
A --> D[".env<br/>环境变量"]
A --> E[".lark2agent_state.json<br/>运行状态文件"]
A --> F["日志文件<br/>lark2agent_ws.log"]
B --> G["Lark 机器人"]
C --> H["本地工程环境"]
```
...
- [lark2agent_ws.py:342-386](file://lark2agent_ws.py#L342-L386)
- [lark2agent_ws.py:409-499](file://lark2agent_ws.py#L409-L499)
- [DEVELOPMENT.md:21-31](file://DEVELOPMENT.md#L21-L31)

## 架构总览
Lark2Agent 的本地运行模型由多线程组成：WebSocket 事件线程、事件队列工作者、调度器、会话观察器、macOS 保活循环、Agent 任务线程与 Plan 调度线程。所有配置通过环境变量注入，运行状态写入本地文件。
...
**图示来源**
- [DEVELOPMENT.md:21-31](file://DEVELOPMENT.md#L21-L31)
- [lark2agent_ws.py:8559-8567](file://lark2agent_ws.py#L8559-L8567)
...
**图示来源**
- [lark2agent_ws.py:3101-3165](file://lark2agent_ws.py#L3101-L3165)
- [lark2agent_ws.py:342-386](file://lark2agent_ws.py#L342-L386)
...
- [lark2agent_ws.py:140-144](file://lark2agent_ws.py#L140-L144)
...
- [lark2agent_ws.py:142-143](file://lark2agent_ws.py#L142-L143)
...
## 结论
Lark2Agent 的开发环境以“单脚本 + 环境变量 + 本地状态文件”为核心，配合 Lark SDK 与 Agent CLI 实现远程控制、协作可见与可审批的工程任务执行。遵循本指南的环境准备、IDE 配置与开发流程，可快速搭建稳定可迭代的开发环境。
...
- VSCode
  ...
  - 设置任务/调试：使用 launch.json 调用 python3 lark2agent_ws.py
- PyCharm
  - 项目解释器选择虚拟环境
