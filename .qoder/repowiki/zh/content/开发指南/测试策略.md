...
## 简介
本测试策略文档面向 Lark2Agent 的单元测试与集成测试，围绕以下目标展开：
...
## 项目结构
Lark2Agent 主程序为单文件脚本，核心逻辑集中在 lark2agent_ws.py，包含：
- 配置与环境变量解析
- 数据模型与运行时状态
...
## 核心组件
- 配置与环境变量：集中于 lark2agent_ws.py 的全局配置解析与常量定义，涵盖 Lark、Agent CLI、附件、计划、审批、状态推送、macOS 保活等。
...
- 状态持久化：.lark2agent_state.json 的读写、裁剪与权限设置。
...
## 架构总览
Lark2Agent 采用单进程 WebSocket 客户端模型，主线程包含事件处理、调度、会话监听与保活循环。业务逻辑通过适配器抽象 Agent CLI，计划任务通过 Git worktree 实现隔离，状态通过本地文件持久化。
...
- 测试数据准备
  ...
  - 使用临时目录承载 .lark2agent 状态文件与附件目录，确保可重复与隔离。
...
章节来源
- [lark2agent_ws.py:1109-1150](file://lark2agent_ws.py#L1109-L1150)
- [lark2agent_ws.py:3101-3165](file://lark2agent_ws.py#L3101-L3165)
- [lark2agent_ws.py:1283-1291](file://lark2agent_ws.py#L1283-L1291)

### 测试环境搭建
- 测试数据库配置
  - 本项目无内置数据库，状态通过 .lark2agent_state.json 文件持久化。测试中使用临时目录隔离状态文件。
- 模拟服务设置
  - 使用 unittest.mock 替换：
    ...
    - Path 与文件系统操作（附件目录）
- 测试数据管理
  - 附件目录：.lark2agent/attachments，相对工作目录，避免写入生产环境。
  - 状态文件：.lark2agent_state.json，测试前后清理，避免污染。
...
PLAN["计划模块"] --> GIT["Git"]
STATE["状态模块"] --> FS[".lark2agent_state.json"]
...
章节来源
- [lark2agent_ws.py:109-127](file://lark2agent_ws.py#L109-L127)
- [lark2agent_ws.py:1449-1539](file://lark2agent_ws.py#L1449-L1539)
- [lark2agent_ws.py:869-891](file://lark2agent_ws.py#L869-L891)
...
## 结论
本测试策略围绕 Lark2Agent 的核心模块，提出从单元到集成的完整测试方案。通过 Mock 与事件模拟，结合覆盖率与质量标准，可在不依赖真实 Lark 环境与 Agent CLI 的前提下，高效验证关键路径、边界条件与异常处理。配合 CI 流程，可保障代码演进过程中的稳定性与可维护性。
