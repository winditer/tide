...
Lark2Agent 是一个将飞书/Lark 群聊消息转换为 Codex、Claude Code 等 Agent CLI 任务的桥梁系统。它通过 WebSocket 长连接接收 Lark 消息，将自然语言指令转换为 Agent CLI 命令执行，并将执行进度、任务结果、审批操作和项目状态同步回 Lark 卡片。

该系统专为工程团队设计，提供了一个轻量级的多 Agent 桥接解决方案，支持后台运行、任务审批、并行执行和状态同步等功能。

## 项目结构

Lark2Agent 项目采用简洁的单文件架构设计：

```mermaid
graph TB
subgraph "项目根目录"
A[lark2agent_ws.py<br/>主程序文件]
B[README.md<br/>项目文档]
C[PPT_OUTLINE.md<br/>演示大纲]
D[.env.example<br/>环境变量模板]
E[.lark2agent_state.json<br/>运行状态文件]
end
...
- [lark2agent_ws.py:173-406](file://lark2agent_ws.py#L173-L406)

## 架构概览

Lark2Agent 采用事件驱动的架构模式，通过 WebSocket 与 Lark 平台实时交互：
...
- [lark2agent_ws.py:674-765](file://lark2agent_ws.py#L674-L765)
...
**图表来源**
- [lark2agent_ws.py:1-31](file://lark2agent_ws.py#L1-L31)
...
- [lark2agent_ws.py:1717-1770](file://lark2agent_ws.py#L1717-L1770)

## 结论

Lark2Agent 提供了一个轻量级但功能完整的 Lark 到 Agent CLI 桥接解决方案。其简洁的架构设计、灵活的配置系统和强大的功能集使其成为工程团队的理想选择。
...
#### 监控和维护
- 查看日志：`tail -f lark2agent_ws.log`
- 查询进程：`pgrep -fl lark2agent_ws.py`
- 停止进程：`pkill -f lark2agent_ws.py`