...
function AgentNodeImpl({ data, selected }: NodeProps) {
  ...
  const label = String(d.label ?? "Agent");
  ...
  return (
    ...
    >
      ...
      {/* Status stripe */}
      <div
        className={`flex items-center justify-between px-3 py-1 ${t.accent} text-white ${t.pulse}`}
      >
      ...
      <div className={`relative px-3 py-2.5 ${t.surface}`}>
        ...
        {prompt && (
          <div className="mt-2 line-clamp-2 border-t border-dashed border-zinc-300 pt-2 font-mono text-[10px] leading-relaxed text-zinc-600">
            {prompt}
          </div>
        )}
      </div>
